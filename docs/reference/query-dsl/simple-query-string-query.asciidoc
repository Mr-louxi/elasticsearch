...
Returns documents based on a provided query string, using a limited,
...
notation. See <<simple-query-string-multi-field>> for an example.

Defaults to the `index.query.default_field` index setting, which has a default
value of `*`. The `*` value extracts all fields in the mapping that are eligible
...
(all operators). See <<supported-flags>> for valid values and syntax.
...
[[simple-query-string-multi-field]]
...
==== Synonyms and the `simple_query_string` query
