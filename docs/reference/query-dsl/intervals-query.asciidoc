...
`pattern`::
(Required, string) Wilcard pattern used to find matching terms.
+
--
...
intervals produced by another rule. The following search returns
...
prefix of the other, then the longer rule can never be matched, which can
